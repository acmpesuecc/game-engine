...
SDL_AppResult SDL_AppInit(void **appState, int argc, char *argv[]) {
  ...
  SDL_SetAppMetadata("GameEngine", "0.0.1", "org.acm.pesuecc.aiep.game-engine");
     ...
     data.keyboard.modifiers);
      ...
      gameEngine->GetEvents().RegisterCallback(
          Engine::EventType::KeyUp, [](Engine::EventData data) {
              ...
              SPDLOG_INFO("Key released: {} (scancode)", static_cast<unsigned
     int>(data.keyboard.scancode)); SPDLOG_INFO("Key released modifiers: {}",
     data.keyboard.modifiers);
          });
      */
  gameEngine->GetEvents().RegisterCallback(
  ...
  gameEngine->GetEvents().RegisterCallback(
      Engine::EventType::KeyDown, [gameEngine](Engine::EventData data) {
        ...
        case 1:
          rect = gameEngine->GetRenderManager().GetRenderable("Rectangle1");
          break;
        case 2:
          rect = gameEngine->GetRenderManager().GetRenderable("Rectangle2");
          break;
        case 3:
          rect = gameEngine->GetRenderManager().GetRenderable("Rectangle3");
          break;
        case 4:
          rect = gameEngine->GetRenderManager().GetRenderable("Rectangle4");
          break;
        }
        if (data.keyboard.keycode == SDLK_RIGHT) {
          ...
          auto &layer =
              gameEngine->GetRenderManager().GetLayer(s_currentRect * 100);
